...
These are the **single source of truth**. If no plugin-root variable is available, locate `claude/instructions/model-roles.md`, `claude/instructions/profile-strict.md`, and `claude/instructions/git-identity.md` inside the `vt` plugin directory and read all three. Do not restate the policy from memory; read the files so this skill never drifts. The performance variant is the `systemprompt` skill (core + `profile-performance.md`) — do not merge the two.
