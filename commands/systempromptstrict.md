...
These three files are the **single source of truth** for this command. Do not restate the policy from memory; read all three files so this command never drifts. The performance variant lives in `/vt:systemprompt` (core + `profile-performance.md`) — do not merge the two.
