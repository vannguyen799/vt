...
Read ALL FOUR files and adopt them together as the active operating policy:
...
These four files are the **single source of truth** for this command — the same core the `vt` `SessionStart` hook injects and the `systemprompt` skill reads, plus the performance profile. Do not restate the policy from memory; read all four files so this command never drifts. The strict / cost-optimized variant lives in `/vt:systempromptstrict` (core + `profile-strict.md`) — do not merge the two.
